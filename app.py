...
# Title and Logo
st.image('images.jpeg', width=450, caption="Skill Share Verify™")
st.title("Skill Share Verify™")
...
if st.session_state.submitted:
    ...
    st.markdown("### Skill Share Verify™ Assignment Verification application by Rohit Krishnan")
            ...
            with st.spinner("🧠 Analyzing..."):
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp:
                    ...
                    st.download_button(
                        "Download PDF Report",
                        pdf_bytes,
                        file_name=f"BIA_ScholarVerify_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
...
def show_trainer_dashboard():
    ...
    for i, row in df.iterrows():
        ...
        cols[2].write(row["Institution"])
        cols[3].write(row["Question Summary"])
        cols[4].write(row["Score"])
        cols[5].write(row["Evaluation Result"])
